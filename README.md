...
Если вы проверяете отдельные части, сгенерированные поэтапно, добавьте `--allow-partial` — в этом режиме валидатор
не будет требовать присутствия всех функций/секций, но продолжит проверять структуру и заполненность блоков:

```bash
python3 valid.py path/to/file.go tmp/003-func-newtask.md --allow-partial
```
