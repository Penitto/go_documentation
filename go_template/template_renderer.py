...
from .anchor_utils import function_anchor_fragment
...
def _relation_link(label: str) -> str:
    ...
    if match:
        ...
        target = f"{_doc_path_from_label(file_label)}#{function_anchor_fragment(name)}"
    ...
    if ":" in label:
        ...
        if left.endswith(".go") or "/" in left:
            target = f"{_doc_path_from_label(left)}#{function_anchor_fragment(right or label)}"
            return f"[{label}]({target})"
    target_name = _normalize_relation_target_name(label)
    target = f"#{function_anchor_fragment(target_name)}"
    return f"[{label}]({target})"
