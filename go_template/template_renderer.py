...
def render_template(
    ...
    if funcs:
        ...
        for func in funcs:
            ...
            lines.append(f"- Назначение: <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Входные данные: `{param_display}` — <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Выходные данные: `{return_display}` — <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Внутренние переменные: <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Внутренняя логика: <<FILL {str(uuid.uuid4())[:8]}>>")
            ...
            if other_rel != "—":
                for sub_line in other_rel.splitlines():
                    lines.append(f"  {sub_line}")
            lines.append(f"- Связь с бизнес-процессом: <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Предусловия: <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append(f"- Постусловия: <<FILL {str(uuid.uuid4())[:8]}>>")
            lines.append("")
    else:
